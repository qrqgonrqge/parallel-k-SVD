...
def q_j_and_R(Q, D_k, j, R):
    ...
    return ortho_comp(D_k, R[:, 0:j, j], Q, j)
...
def stability_fix(q_j, global_dead_batches, atom_mask, k, Q, R, j):
    ...
    q_j_norm_safe[dead_batches] = 1.0 # Avoid division by zero
...
def find_best(r, D, atom_mask,
    ...
    # atom_bookeeping2(k, atom_mask, batch_idx, I, j, D, D_k) # worse
...
def OMP(Y, T_0, D, batch_size = 1, rng=42, debug=False, float_dtype=np.float32):
    ...
    for i, split in enumerate(splits):
        ...
        k = np.empty(batch_size, dtype=int_dtype)
