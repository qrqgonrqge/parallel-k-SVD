...
Distorter::Distorter(const Eigen::MatrixXf& image, float noise_level, unsigned int seed)
...
Eigen::MatrixXf Distorter::extractNormalizedPatches(int patch_size) const {
    ...
    Eigen::MatrixXf patches(n_patches, patch_len);
    ...
    for (int j = 0; j < patch_len; j++) {
        float mean = patches.col(j).mean();
        patches.col(j).array() -= mean;
        float std = std::sqrt(patches.col(j).squaredNorm() / patches.rows());
        if (std > 1e-7f)
            patches.col(j) /= std;
    }
