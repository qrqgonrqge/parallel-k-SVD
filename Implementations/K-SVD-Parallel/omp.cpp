...
bool OMP::do_omp() {
    ...
    // Each batch is fully independent: reads Y and D (shared, read-only),
    // writes to non-overlapping rows of X. All batch-local state is declared
    // inside the loop body so it is automatically thread-private.
