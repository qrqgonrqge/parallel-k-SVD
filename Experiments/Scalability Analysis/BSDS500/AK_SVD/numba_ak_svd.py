...
def OMP_baseline(Y, T_0, D, batch_size = 1, rng=42, debug=False, float_dtype=np.float32):
    int_dtype = np.int32
    ...
    for i, y in enumerate(Y_batches):
        ...
        for j in range(T_0):
            ...
            if j == 0:
                ...
                dot = np.einsum('bjd, bd -> bj', Q[:, :j], D_k, dtype=float_dtype)
                ...
                q_j = D_k - np.einsum('bj, bjd -> bd', dot, Q[:, :j], dtype=float_dtype)
                
                
                # norm, axis = 1: (B, D.shape[1]) -> (B,)
                # q_j_norm = np.linalg.norm(q_j, axis = 1)
                q_j_norm = np.sqrt(
                            np.einsum('bd, bd -> b', q_j, q_j, dtype=float_dtype), 
                            dtype=float_dtype)
            ...
            y_proj = np.einsum('bd, bd -> b', y, Q[:, j], dtype=float_dtype)
...
def kSVD_baseline(Y, T_0, k, num_iter, 
         batch_size = 1, 
         track_loss = True, 
         verbose:int = 0, 
         rng = 42,
    ...
    for iter in range(num_iter):
        ...
        X = OMP_baseline(Y, T_0, D, 
                batch_size = batch_size, 
                rng=rng, 
                debug=(verbose > 0), 
                float_dtype=dtype)
        if verbose > 0:
            print(f'\tCoding Time: {time.time() - t0}')
        ...
        loss[iter] = np.linalg.norm(E_k_R, ord='fro')
...
def OMP(Y, T_0, D, batch_size = 1, rng=42, debug=False, float_dtype=np.float32):
    ...
    global_batch_idx = np.arange(batch_size, dtype=int_dtype)
    ...
    for i in prange(len(splits)):
        ...
        batch_idx = global_batch_idx[:curr_batch_size]

        y = Y[start_idx:end_idx]
                
        I = np.empty((curr_batch_size, T_0), dtype=int_dtype)
        D_I = np.zeros((curr_batch_size, T_0, D.shape[1]), dtype=float_dtype)

        # Deep copy to not overwrite Y
        # r = y.copy()   # (curr_batch_size, D.shape[1])
        # r = y.astype(float_dtype)
        r = res[start_idx:end_idx]
        gamma = 0
        
        # Q = np.empty((curr_batch_size, D.shape[1], T_0))
        Q = np.empty((curr_batch_size, T_0, D.shape[1]), dtype=float_dtype)
        
        Q_T_y = np.empty((curr_batch_size, T_0)[::-1], dtype=float_dtype).T
        # R = np.empty((curr_batch_size, T_0, T_0))
        R = np.zeros((curr_batch_size, T_0, T_0)[::-1], dtype=float_dtype).T
        

        # Create a mask to ensure duplicates aren't selected
        atom_mask = np.ones((curr_batch_size, D.shape[0]), dtype=float_dtype)

        global_dead_batches = np.zeros(curr_batch_size, dtype=np.bool_)
        j_stop = T_0
        ...
        gamma = np.empty((curr_batch_size, j_stop), dtype=float_dtype)
        for n in range(1, j_stop+1):
            idx = j_stop - n
            gamma_idx = Q_T_y[:, idx] / R[:, idx, idx]
            gamma[:, idx] = gamma_idx
            for l in range(idx):
                Q_T_y[:, l] -= gamma_idx * R[:, l, idx]
