...
def run_ksvd_benchmark(Y, T_0, k, num_iter, threads_list, batch_size=1, weak=True):
    ...
    df.to_csv(f"{output_dir}/ksvd_weakWEAKWEAKWEAKWEKAWEKWAEKAWE_scaling_N{Y.shape[1]}.csv", index=False)
    return df
...
if __name__ == "__main__":
    ...
    benchmark_data = run_ksvd_benchmark(Y_data, T_0=32, k=256, num_iter=10, threads_list=thread_counts, batch_size=128, weak=True)
