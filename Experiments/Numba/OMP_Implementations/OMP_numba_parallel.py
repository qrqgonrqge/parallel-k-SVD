...
# @njit(parallel=True)
@jit
def OMP_numba_jit(Y,T_0,D,batch_size = 1, seed=42, debug=False):
    '''
    nopython jitted OMP with parallel True and prange explicitly set. Best performance given overhead doesn't exceed sequential case.
    Future Improvements: for loops instead of list comprehension, replacing special indexing with for loops
    '''
...
@njit
def OMP_numba_njit(Y,T_0,D,batch_size = 1, seed=42, debug=False):
    '''
    nopython jitted OMP with parallel True and prange explicitly set. Best performance given overhead doesn't exceed sequential case.
    Future Improvements: for loops instead of list comprehension, replacing special indexing with for loops
    '''
...
@njit(parallel=True)
def OMP_numba_njit_parallel(Y,T_0,D,batch_size = 1, seed=42, debug=False):
    '''
    nopython jitted OMP with parallel True and prange explicitly set. Best performance given overhead doesn't exceed sequential case.
    Future Improvements: for loops instead of list comprehension, replacing special indexing with for loops
    '''
...
@njit(parallel=True)
def OMP_numba_njit_parallel_pbatches(Y,T_0,D,batch_size = 1, seed=42, debug=False):
    '''
    nopython jitted OMP with parallel True and prange explicitly set. Best performance given overhead doesn't exceed sequential case.
    Future Improvements: for loops instead of list comprehension, replacing special indexing with for loops
    '''
...
if __name__ == "__main__":
    ...
    if timing:
        N = 10
        OMP_serial(y.T, T0, D.T, batch_size=batch_size) # Warmup
        print('OMP_serial:', timeit.timeit(lambda: OMP_serial(y.T, T0, D.T, batch_size=batch_size), number=N)/N, 's')

        # OMP_numba_serial(y.T, T0, D.T, batch_size=batch_size)
        # print('OMP_numba_serial:', timeit.timeit(lambda: OMP_numba_serial(y.T, T0, D.T, batch_size=batch_size), number=N)/N, 's')

        # OMP_numba_jit(y.T, T0, D.T, batch_size=batch_size)
        # print('OMP_numba_jit:', timeit.timeit(lambda: OMP_numba_jit(y.T, T0, D.T, batch_size=batch_size), number=N)/N, 's')

        # OMP_numba_njit(y.T, T0, D.T, batch_size=batch_size)
        # print('OMP_numba_njit:', timeit.timeit(lambda: OMP_numba_njit(y.T, T0, D.T, batch_size=batch_size), number=N)/N, 's')

        # OMP_numba_njit_parallel(y.T, T0, D.T, batch_size=batch_size)
        # print('OMP_numba_njit_parallel:', timeit.timeit(lambda: OMP_numba_njit_parallel(y.T, T0, D.T, batch_size=batch_size), number=N)/N, 's')

        OMP_numba_njit_parallel_pbatches(y.T, T0, D.T, batch_size=batch_size)
        print('OMP_numba_njit_parallel_pbatches:', timeit.timeit(lambda: OMP_numba_njit_parallel_pbatches(y.T, T0, D.T, batch_size=batch_size), number=N)/N, 's')

        OMP_numba_njit_parallel_optimized(y.T, T0, D.T, batch_size=batch_size)
        print('OMP_numba_njit_parallel_optimized:', timeit.timeit(lambda: OMP_numba_njit_parallel_optimized(y.T, T0, D.T, batch_size=batch_size), number=N)/N, 's')
